...
def eglif_solver(t, I, Ge, Gi,
    ...
    returns : activity vm, time instant of the spikes t_spikes, adaptation w_vett
    (and other suffs)
    """
...
def plot_eglif(t, V, tspikes):

    fig = make_subplots(rows=1, cols=1, subplot_titles=(''))

    fig.add_trace(go.Scatter(x=t, y=V, mode='lines', name='Vm',  line=dict(color = "orchid")),
                      row=1, col=1)
    ...
    fig.update_layout(title='# of spikes: '+str(len(tspikes)),
                          xaxis_title='t [s]',yaxis_title='Vm [V]',)
    fig.show()
